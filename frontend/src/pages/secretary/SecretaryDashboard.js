  ...
  const columns = [
    ...
    { title: '状态', dataIndex: 'status', render: s => <Tag color={s === '复审中' ? 'blue' : (s === '待初审' ? 'orange' : 'default')}>{s}</Tag> },
    {
      ...
      render: (_, record) => (
        <Space>
          {record.status === '待初审' && (
            <Button type="primary" size="small" icon={<AuditOutlined />} onClick={() => setAuditModal({ open: true, projectId: record.project_id })}>初审</Button>
          )}
          {record.status === '复审中' && (
            <Button size="small" icon={<SendOutlined />} onClick={() => setAssignModal({ open: true, projectId: record.project_id })}>分配</Button>
          )}
        </Space>
      ),
    },
