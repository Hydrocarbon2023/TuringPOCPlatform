...
const UserDashboard = () => {
  ...
  const projectColumns = [
    {title: '项目名称', dataIndex: 'project_name', render: t => <b style={{color: token.colorTextHeading}}>{t}</b>},
    {
      ...
      render: n => <span style={{color: token.colorTextSecondary}}>{n}</span>
    },
  ...
  const teamColumns = [
    ...
    {
      title: '操作',
      render: (_, r) => r.role === '队长' && <Button size="small" icon={<UserAddOutlined/>}
    ...
    <div style={{display: 'flex', flexDirection: 'column', gap: 24}}>
      ...
      <Card style={{borderRadius: token.borderRadius}}>
        <Tabs items={[
          {
            key: '1',
            label: <span><ProjectOutlined/> 项目列表</span>,
            children: <Table dataSource={projects} columns={projectColumns} rowKey="project_id"/>
          },
          {
            key: '2',
            label: <span><TeamOutlined/> 我的团队</span>,
            children: <Table dataSource={myTeams} columns={teamColumns} rowKey="team_id"/>
          }
        ]}/>
      </Card>
      ...
      <Modal title="邀请成员" open={inviteModal.open} footer={null}
      ...
      <Modal title="项目详情" open={detailModal.open} onCancel={() => setDetailModal({open: false, data: null})}
            ...
            {detailModal.data.review_info && detailModal.data.review_info.avg_score > 0 && (
              <Descriptions.Item label="复审结果">
                <div style={{padding: 12, background: '#f6ffed', border: '1px solid #b7eb8f', borderRadius: 6}}>
                  <div style={{marginBottom: 4}}>专家评审均分：<b
                    style={{fontSize: 20, color: '#135200'}}>{detailModal.data.review_info.avg_score}</b> 分
                  </div>
                  <div
                    style={{color: '#888', fontSize: 12}}>（基于 {detailModal.data.review_info.review_count} 位专家的评分）
                  </div>
