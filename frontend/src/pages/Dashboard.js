...
const Dashboard = () => {
  ...
  return (
    <div>
      ...
      >
        <Table rowKey={role === '评审人' ? "task_id" : "project_id"}
          loading={loading}
          dataSource={data}
          columns={
            role === '项目参与者' ? participantColumns : (role === '秘书' ? secretaryColumns : expertColumns)
          }
        />
      </Card>

      <Modal title={currentAction === 'create' ? "申报项目" : (currentAction === 'audit' ? "项目初审" : "项目复审")}
        open={modalVisible}
        onOk={handleSubmit}
